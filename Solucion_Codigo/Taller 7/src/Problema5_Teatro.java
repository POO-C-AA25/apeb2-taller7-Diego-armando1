...
public class Problema5_Teatro {
    ...
    public static Entrada venderEntrada(String zonaNombre, String comprador, String tipo){
        ...
        switch (tipo.toLowerCase()){
            case "normal" :
                entrada = new EntradaNormal(comprador, zona);
                break;
            case "reducida" :
                entrada = new EntradaReducida(comprador, zona);
                break;
            case "abonado" : 
                entrada = new EntradaAbonado(comprador, zona);
                break;
            default:
        ...
        System.out.println("Entrada vendida: ID " + entrada.id() + " Precio: " + entrada.precio());
        return entrada;
    }
    public static void consultarEntrada(int id){
        for (Entrada d : entradas){
            if (d.id() == id){
